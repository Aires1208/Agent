...
import com.navercorp.pinpoint.bootstrap.util.ServerTypeUtils;
...
public class AgentInformationFactory {
    ...
    public AgentInformation createAgentInformation(ServiceType serverType) {
        ...
        final String agentId = getId("smartsight.agentId", machineName, PinpointConstants.AGENT_NAME_MAX_LEN);
        final String applicationName = getId("smartsight.applicationName", "UnknownApplicationName", PinpointConstants.APPLICATION_NAME_MAX_LEN);
        ...
        final String mac = NetworkUtils.getMac();
        final String os = NetworkUtils.getOs();
        final boolean isDocker = ServerTypeUtils.isDocker();
        return new AgentInformation(agentId, applicationName, startTime, pid, machineName, hostIp, serverType, jvmVersion, Version.VERSION, mac, os,isDocker);
    }
