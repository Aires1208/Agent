...
public class DefaultProfilerConfig implements ProfilerConfig {
    ...
    private String logstashIp = DEFAULT_IP;
    ...
    private int kafkaTcpPort = 9993;

    private String collectorIp = DEFAULT_IP;
    ...
    void readPropertyValues() {
        ...
        String systemType = readFromSystemProperty("systemType","default");

        if("paas".equals(systemType)){
            MsbHelper msbHelper = new MsbHelper();

            this.logstashIp = msbHelper.getMsbAdds();

            this.collectorIp = msbHelper.getMsbAdds();

            this.collectorSpanServerPort = msbHelper.getServicePublishPort("smartsight-logstash-span",29996);

            this.collectorStatServerPort = msbHelper.getServicePublishPort("smartsight-logstash-stat",29995);

            this.collectorTcpServerPort = msbHelper.getServicePublishPort("smartsight-collector-tcp",29994);

            this.kafkaTcpPort = msbHelper.getServicePublishPort("smartsight-logstash-tcp",29993);

        }else{
            this.logstashIp = readFromSystemProperty(LOGSTASH_IP, DEFAULT_IP);

            this.collectorIp = readFromSystemProperty(COLLECTOR_IP, DEFAULT_IP);
            ...
            this.kafkaTcpPort = readInt("profiler.kafka.tcp.port", 9993);
        }
    ...
    private String readFromSystemProperty(String key, String defaultValue) {
