...
public class ClassPathResolverTest {
    ...
    public void testFindAgentJar() throws Exception {
        ...
        Assert.assertEquals("smartsight-bootstrap-0.0.2.jar", agentJar);

        String agentPath = classPathResolver.getAgentJarFullPath();
        Assert.assertEquals("D:\\nhn_source\\pinpoint_project\\deploy\\agent\\agentlib/smartsight-bootstrap-0.0.2.jar", agentPath);
    ...
    public void testFindAgentSnapshotJar() throws Exception {
        String path = "D:\\nhn_source\\pinpoint_project\\deploy\\apache-tomcat-6.0.35\\bin\\bootstrap.jar;D:\\nhn_source\\pinpoint_project\\deploy\\agent\\agentlib/lib/javassist-3.16.1.GA.jar;" +
                ...
                "/smartsight-bootstrap-0.0.2-SNAPSHOT.jar";
        // D:\nhn_source\pinpoint_project\deploy\agent\agentlib/pinpoint-tomcat-profiler-0.0.2.jar
        ClassPathResolver classPathResolver = new ClassPathResolver(path);
        boolean findAgentJar = classPathResolver.findAgentJar();
        Assert.assertTrue(findAgentJar);
        ...
        String agentJar = classPathResolver.getAgentJarName();
        Assert.assertEquals("smartsight-bootstrap-0.0.2-SNAPSHOT.jar", agentJar);

        String agentPath = classPathResolver.getAgentJarFullPath();
        Assert.assertEquals("D:\\nhn_source\\pinpoint_project\\deploy\\agent\\agentlib/smartsight-bootstrap-0.0.2-SNAPSHOT.jar", agentPath);
    ...
    public void findAgentJar() {
        findAgentJar("smartsight-bootstrap-0.0.2.jar");
        findAgentJar("smartsight-bootstrap-1.0.0.jar");
        findAgentJar("smartsight-bootstrap-1.10.20.jar");
        findAgentJar("smartsight-bootstrap.jar");


        findAgentJarAssertFail("smartsight-bootstrap-1.a.test.jar");
        findAgentJarAssertFail("smartsightbootstrap-1.a.test.jar");
        findAgentJarAssertFail("smartsightbootstrap.jar");
    }
