...
public class PinpointStarter {
    ...
    public void start(String bootClass) {
        ...
        if (!isValidId("smartsight.agentId", PinpointConstants.AGENT_NAME_MAX_LEN)) {
            logPinpointAgentLoadFail();
            return;
        }
        if (!isValidId("smartsight.applicationName", PinpointConstants.APPLICATION_NAME_MAX_LEN)) {
            logPinpointAgentLoadFail();
            return;
        }
    ...
    private void saveLogFilePath(ClassPathResolver classPathResolver) {
        String pinpointLogName = ProductInfo.NAME + ".log";
        String agentLogFilePath = System.getProperty(pinpointLogName);
        if (agentLogFilePath == null) {
            agentLogFilePath = classPathResolver.getAgentLogFilePath();
        }
        System.out.println("logPath:" + agentLogFilePath);
        logger.info("logPath:" + agentLogFilePath);

        systemProperty.setProperty(pinpointLogName, agentLogFilePath);
    }
