...
public final class TransactionIdUtils {
    ...
    public static byte[] formatBytes(TransactionId transactionId) {
        return formatBytes(transactionId.getAgentId(), transactionId.getAgentStartTime(), transactionId.getTransactionSequence());
    }

    public static byte[] writeTraceIdIndexValue(Set<TransactionId> transactionIdSet) {
        final Buffer buffer = new AutomaticBuffer();
        byte version = 0;
        buffer.put(version);
        buffer.put(transactionIdSet.size());
        for (TransactionId transactionId : transactionIdSet) {
