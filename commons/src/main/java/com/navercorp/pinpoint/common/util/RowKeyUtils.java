...
public final class RowKeyUtils {
    ...
    public static byte[] createTimeSlotRowKey(String name, long timeslot) {
        byte[] nameKey = BytesUtils.toBytes(name);
        long reverseStartTimestamp = TimeUtils.reverseTimeMillis(timeslot);
        return concatFixedByteAndLong(nameKey, APPLICATION_NAME_MAX_LEN, reverseStartTimestamp);
    }
