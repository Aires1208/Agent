...
public final class SpanUtils {
    ...
    public static TransactionId getTransactionIdObj(TSpan span) {
        if (null == span) {
            throw new NullPointerException("span must not be null.");
        }

        byte[] bytesTraceId = span.getTransactionId();
        TransactionId traceId = TransactionIdUtils.parseTransactionId(bytesTraceId);
        String agentId = traceId.getAgentId();
        if (null == agentId) {
            return new TransactionId(span.getAgentId(), traceId.getAgentStartTime(), traceId.getTransactionSequence());
        }
        return traceId;
    }
}
